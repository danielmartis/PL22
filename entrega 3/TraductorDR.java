...
class TraductorDR{
    ...
    public String Instr(String tipo){
        if(token.tipo == Token.INT || token.tipo == Token.FLOAT){
            ...
            if(expr.atrib.equals("entero") && s.tipo == 2){
                return s.nomtrad + ":= itor" + expr.atrib + ";\n"; 
            }
    ...
    public Atributos Expr(){
        if(token.tipo == Token.NUMREAL || token.tipo == Token.NUMINT || token.tipo == Token.ID || token.tipo == Token.PARI){
            ...
            if(exprp.atrib.equals(vacio)){
                ...
                if(term.atrib.equals("entero") && exprp.atrib.equals("real")){
                    return new Atributos("itor(" + term.trad + ")" + exprp.trad, "real"); 
                }
                else if(term.atrib.equals("real") && exprp.atrib.equals("entero")){
                    return new Atributos(term.trad + " itor(" + exprp.trad + ")", "real");
                }
                return new Atributos(term.trad + exprp.trad, term.atrib);
            }
        }
    ...
    public Atributos Exprp(String t){
        if(token.tipo == Token.OPAS){
            ...
            exprp = Exprp(term.atrib);
            if(exprp.atrib.equals(vacio)){
                if(term.atrib.equals("entero")){
                    if(t.equals("real")){
                        return new Atributos(" " + si + "r itor(" + term.trad + ")", "real");
                    }
                    return new Atributos(" " + si + "i "  + term.trad, term.atrib);
                }
                else{
                    return new Atributos(" " + si + "r " + term.trad, term.atrib);
                }
            }
            if(exprp.atrib.equals(term.atrib)){
                if(term.atrib.equals("entero")){
                    if(t.equals("real")){
                        return new Atributos(" " + si + "r itor(" + term.trad  +")" + exprp.trad, "real");
                    }
                    return new Atributos(" " + si + "i " + term.trad + exprp.trad, term.atrib);
                }
                else{
                    return new Atributos(" " + si + "r " + term.trad  + exprp.trad, term.atrib);
                }
                ...
                if(term.atrib.equals("entero") && exprp.atrib.equals("real")){
                    ...
                    return new Atributos(" " + si + "r " + term.trad + " itor(" + exprp.trad + ")", "real");
                }
            }
        }
    ...
    public Atributos Term(String t){
        if(token.tipo == Token.NUMREAL || token.tipo == Token.NUMINT || token.tipo == Token.ID || token.tipo == Token.PARI){
            ...
            if(termp.trad.equals(vacio)){
                return new Atributos(factor.trad, factor.atrib);
            }
            if(factor.atrib.equals(termp.atrib)){
                if(termp.atrib.equals("entero")){
                    return new Atributos(factor.trad +  termp.trad, termp.atrib);
                }
                else{
                    return new Atributos(factor.trad +  termp.trad, termp.atrib);
                }
                ...
                if(factor.atrib.equals("entero") && termp.atrib.equals("real")){
                    return new Atributos("itor(" + factor.trad + ")"  +  termp.trad, "real"); 
                }
                else{
                    return new Atributos(factor.trad + "r itor(" + termp.trad + ")", "real");
                }
            }
        }
        else{
    ...
    public Atributos Termp(String t){
        if(token.tipo == Token.OPMD){
            ...
            factor = Factor();
            termp = Termp(factor.atrib);
            if(termp.atrib.equals(vacio)){
                if(factor.atrib.equals("entero")){
                    if(t.equals("real")){
                        return new Atributos(" " + si + "r itor("  + factor.trad + ")", "real");
                    }
                    else{
                        return new Atributos(" " + si + "i "  + factor.trad, factor.atrib);
                    }
                }
                else{
                    return new Atributos(" " + si + "r " + factor.trad, factor.atrib);
                }
            }   
            if(factor.atrib.equals(termp.atrib)){
                if(termp.atrib.equals("entero")){
                    if(t.equals("real")){
                        return new Atributos(" " + si + "r itor("  + factor.trad + termp.trad + ")", "real");
                    }
                    else{
                        return new Atributos(" " + si + "i "  + factor.trad + termp.trad, factor.atrib);
                    }
                }
                else{
                    //System.out.println("Hola");
                    return new Atributos(" " + si + "r " + factor.trad + termp.trad, termp.atrib);
                }
                ...
                if(factor.atrib.equals("entero") && termp.atrib.equals("real")){
                    ...
                    //System.out.println("factor: " + factor.trad + " termp:" + termp.trad);
                    return new Atributos(" " + si + "r " + factor.trad + " itor(" + termp.trad + ")", "real");
                }
            }
        }
