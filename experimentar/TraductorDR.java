...
class TraductorDR{
    ...
    public Atributos Expr(){
        if(token.tipo == Token.NUMREAL || token.tipo == Token.NUMINT || token.tipo == Token.ID || token.tipo == Token.PARI){
            ...
            nregla(27);
            term = Term(vacio, vacio);
            //System.out.println(term.atrib);
            exprp = Exprp(term.atrib, term.trad);
            return exprp;
        }
        else{
            tokE.add(Token.ID);
    ...
    public Atributos Exprp(String t, String tr){
    ...
    public Atributos Term(String t, String tr){
        if(token.tipo == Token.NUMREAL || token.tipo == Token.NUMINT || token.tipo == Token.ID || token.tipo == Token.PARI){
            Atributos factor, termp;
            nregla(30);
            factor = Factor();  
            termp = Termp(factor.atrib, factor.trad);
    ...
    public Atributos Termp(String t, String tr){
